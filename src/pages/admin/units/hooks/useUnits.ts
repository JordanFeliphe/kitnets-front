...
import { Unit, UnitFilter, CreateUnitForm, UnitStatus } from '@/app/shared/types';
import { unitService } from '../services/unitsService';
import { fetchPublicUnits, fetchAdminUnits } from '../services/unitsService';
...
// New TanStack Query hooks for the admin units functionality
export const usePublicUnits = (page?: number, limit?: number) => {
  return useQuery({
    queryKey: ['units', 'public', page, limit],
    queryFn: () => fetchPublicUnits(page, limit),
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
};

export const useAdminUnits = (page?: number, limit?: number) => {
  const { token, user } = useAuth();

  return useQuery({
    queryKey: ['units', 'admin', page, limit],
    queryFn: () => fetchAdminUnits(token!, page, limit),
