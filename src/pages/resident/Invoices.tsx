...
import { ResidentBoletoTable } from "@/components/tables/ResidentBoletoTable"
import { PaymentDrawer } from "@/components/ui/payment-drawer"
...
export default function ResidentInvoices() {
  ...
  const handleDownloadPDF = useCallback((invoice: any) => {
    toast.success(`Baixando boleto ${invoice.numero}...`)
    console.log("Downloading PDF for invoice:", invoice.id)
  }, [])

  const handlePrintInvoice = useCallback((invoice: any) => {
    toast.success(`Preparando boleto ${invoice.numero} para impressão...`)
  ...
  const handleView2ndCopy = useCallback((boleto: any) => {
    ...
    console.log("View 2nd copy:", boleto.id)
  }, [])
