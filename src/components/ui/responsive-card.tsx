...
export const ResponsiveCard: React.FC<ResponsiveCardProps> = ({
  ...
  const renderIcon = (icon: React.ComponentType<any> | React.ReactNode) => {
    ...
    if (import.meta.env.DEV && icon !== null && icon !== undefined) {
      console.warn('ResponsiveCard renderIcon: Unexpected icon type', typeof icon, icon)
    }
    return null
  }
  ...
  const renderFieldValue = (field: ResponsiveCardField) => {
    ...
    switch (field.type) {
      ...
      case 'currency':
        return (
          <span className={cn("font-medium text-green-600", field.className)}>
            R$ {field.value}
          </span>
        )
      case 'date':
